...
static void gameLoop() {
    while (true) {
        ...
        if (dir == Direction::LEFT) playerX -= CG_MOVE_SPEED;
        else if (dir == Direction::RIGHT) playerX += CG_MOVE_SPEED;

        if (playerX < 0) playerX = 0;
        if (playerX > TFT_W - CG_PLAYER_W) playerX = TFT_W - CG_PLAYER_W;

        if (static_cast<int16_t>(playerX) ^ static_cast<int16_t>(oldPX))
            drawPlayer(oldPX, playerX);
...
static void drawHUD() {
    tft.fillRect(0, 0, TFT_W, CG_PLAY_TOP - 3, BG);
    ...
    for (int8_t i = 0; i < playerHP; i++)
        tft.fillRect(HP_X + 22 + i * 12, CG_HUD_Y, 8, 8, HP_ICON_CLR);
    for (int8_t i = playerHP; i < CG_HP_MAX; i++)
        tft.fillRect(HP_X + 22 + i * 12, CG_HUD_Y, 8, 8, BG);

    tft.setTextColor(COIN_CLR, BG);
    tft.setCursor(SCORE_X, CG_HUD_Y);
    tft.print(F("SCORE:"));
    tft.fillRect(SCORE_X + 40, CG_HUD_Y, 60, 8, BG);
...
static void drawItem(const FallingItem &it) {
    uint16_t clr;
    switch (it.type) {
    case ItemType::IT_ENEMY:  clr = ENEMY_CLR;  break;
    case ItemType::IT_HEALTH: clr = HEALTH_CLR; break;
    case ItemType::IT_COIN:   clr = COIN_CLR;   break;
    default:                  clr = TEXT_CLR;   break;
    }
...
static void spawnItem() {
    for (uint8_t i = 0; i < CG_MAX_ITEMS; i++) {
        if (static_cast<uint8_t>(items[i].active) ^ 1) {
            ...
            if (roll < 50) items[i].type = ItemType::IT_ENEMY;
            else if (roll < 65) items[i].type = ItemType::IT_HEALTH;
            else items[i].type = ItemType::IT_COIN;
            return;
        }
    }
}
...
static void handleCollision(FallingItem &it) {
    switch (it.type) {
    case ItemType::IT_ENEMY:  playerHP--; break;
    case ItemType::IT_HEALTH: if (playerHP < CG_HP_MAX) playerHP++; break;
    case ItemType::IT_COIN:   score += CG_COIN_SCORE; break;
    }
}
